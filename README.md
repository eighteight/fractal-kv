...
| `python3 experiments/weighted_chaos.py` | rate vs. the entropy limit; forward/reversed lossy truncation |
| `python3 experiments/multi_span_loss.py` | where truncation damage lands across multiple spans |
| `python3 experiments/grid_walk.py` | grid-vertex walk vs. ring walk: rate, decay law, retrieval |
...
experiments/
  weighted_chaos.py   # probability-weighted maps == arithmetic coding;
                      # rate vs. entropy limit; lossy truncation regimes
  multi_span_loss.py  # damage distribution when many spans are truncated
  grid_walk.py        # GridWalkCodec: vertices on a k x k grid (tiling
                      # cells, exact dyadic decode) vs. the ring walk --
                      # 1.55x smaller storage, same decay law; short-query
                      # retrieval precision is the trade-off
```
